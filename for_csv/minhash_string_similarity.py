...
# TODO: MinHashEnsemble may be better as it normalises intersection by the size of only 
# the query set, so doesn't penalise longer strings.

class MinHash_similarity(object):
    def __init__(self, threshold=0.4, num_perm=128, shingle_length=5, utterance_col='utterance'):
        self.threshold = threshold
        self.num_perm = num_perm
        self.shingle_length = shingle_length
        self.utterance_col = utterance_col

    def import_dataframe(self, df_path, utterance_name='utterance'):
        """
    ...
    def dataframe_to_data_list(self, df, index_prefix):
        """
        ...
        for index, row in df.iterrows():
            m = self.minhash_from_string(row[self.utterance_col])
            temptuple = (index, m)
