...
class kfoldtest(object):
        ...
        for i in range(len(data['intents'])):
            ...
            for j in range(len(data['intents'][i]['examples'])):
                df = df.append({'intent': data['intents'][i]['intent'],
                                'utterance': data['intents'][i]['examples'][j]['utterance']}, ignore_index=True)
    ...
    def intent_df_from_watson(self, workspace_id):
        ...
        response = self.assistant.list_intents(
            workspace_id=workspace_id, page_limit=10000)
        ...
        for i in range(len(data["intents"])):
            ...
            response = self.assistant.list_examples(
                workspace_id=workspace_id, intent=name_intent, page_limit=10000)
            ...
            for j in range(len(data_examples["examples"])):
                ...
                df = df.append(
                    {'intent': name_intent, 'utterance': text}, ignore_index=True)
    ...
    def create_folds(self, method, sample_size=0.2):
        ...
        if method == "kfold":
            ...
            raise ValueError(
                "Parameter method must either be 'kfold' or 'monte-carlo'")
    ...
    def create_intents(self, train_index):
        ...
        for i in train_index:
            ...
            if not any(d['intent'] == intent for d in intent_results):
                ...
                row = [d for d in intent_results if d.get(
                    'intent') == intent][0]
                intent_results[:] = [
                    d for d in intent_results if d.get('intent') != intent]
                e = {'text': text}
    ...
    def test_kfold(self, df_test, ws_id, export_interim=False):
        ...
        for i in tqdm(range(len(df_test))):
            ...
            response = self.assistant.message(
                workspace_id=self.workspaces[ws_id],
                input={'text': text},
                context={
                    'metadata': {
        ...
        if export_interim:
            results.to_csv("./kfold_{}_raw.csv".format(ws_id+1),
                           encoding='utf-8')
    ...
    def get_metrics_per_fold(self, results_kfold):
        ...
        for i in range(len(results_kfold)):
            ...
            incorrect_n = data.loc[data['intent_correct']
                                   != data["expected intent"]]
            incorrect_avg_conf = incorrect_n['confidence1'].mean()
            precision, recall, fscore, support = precision_recall_fscore_support(
                data["intent_correct"], data["expected intent"], average='weighted')
            accuracy = accuracy_score(
                data["intent_correct"], data["expected intent"])
    ...
    def data_prep_confusion_matrix(self, list_df):
        ...
        list_df["intent_correct"] = np.where(
            (list_df["confidence1"] < self.threshold), "BELOW_THRESHOLD", list_df["intent1"])
        matrix = confusion_matrix(
            list_df["intent_correct"], list_df["expected intent"])
