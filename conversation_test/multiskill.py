...
def main(topic_list, conf_matrix, save_master_data):
    ...
    try:
        ...
        if 'apikey' in instance_creds:
            ...
            bs = blindset.blindset(apikey=instance_creds['apikey'], url=instance_creds['url'], version=conversation_version)
        elif 'password' in instance_creds:
            logger.debug("Authenticating (username/password)")
            bs = blindset.blindset(username=instance_creds['username'], password=instance_creds['password'], url=instance_creds['url'], version=conversation_version)
        ...
        # create master train and push to WA
        logger.info("Creating temporary master skill")
        master_train = pd.concat([v.drop(columns=['intent']).assign(intent=k) for k,v in train_dict.items()], axis=0, ignore_index=True, sort=False)
        master_skill_id = wa_utils.create_workspace_from_df(bs.assistant, name='master', train_df=master_train, description='generated by intent_training_tools')
        ...
        # merge master results with topic results 
        for skill in skill_list:
            merged = results_dict[skill].merge(results_master.loc[:, ['original_text', 'r@1', 'intent1']].rename(columns={'r@1':'r@1_master', 'intent1':'intent1_master'}), how='left', on='original_text', suffixes=('_'+skill, '_master'))
            merged['r@1_overall'] = 1*(merged['r@1'] & merged['r@1_master'])

            results_dict[skill] = merged[['original_text', 'r@1_overall', 'r@1_master', 'intent1_master', 'expected intent', 'r@1', 'intent1', 'confidence1', 'intent2',
            'confidence2', 'intent3', 'confidence3']]

        # ..and create subset dfs of utterances that have just been successful on master
        result_subset_dict = {skill: results_dict[skill][results_dict[skill]['r@1_master'] == 1] for skill in skill_list}
        ...
        # calculate metrics
        # master
        met = Metrics(workspace_thresh=Credentials.calculate_workspace_thresh('master'))
        ...
        for skill in skill_list:
            ...
            metrics_dict[skill], _ = met.get_all_metrics(result_subset_dict[skill], detailed_results=True)
...
def check_skills_exist(skill_list):
    ...
    if len(bs_notexist) > 0 & len(cr_notexist) > 0:
        raise ValueError("Blindsets don't exist for topics {}, and Credentials don't exist for topics {}. Please add these and try again".format(bs_notexist, cr_notexist))
    elif len(bs_notexist) > 0:
        raise ValueError("Blindsets don't exist for topics {}. Please add these and try again".format(bs_notexist))
    elif len(cr_notexist) > 0:
        raise ValueError("Credentials don't exist for topics {}. Please add these and try again".format(cr_notexist))
    else:
